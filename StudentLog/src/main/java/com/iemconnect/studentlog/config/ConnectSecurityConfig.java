...
public class ConnectSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception{
       //can set if needto check role and all also
        httpSecurity.csrf(csrf-> csrf.disable())
                .cors(cors->cors.disable())
                .authorizeHttpRequests(auth->auth.requestMatchers("/college/*").authenticated().requestMatchers(
                        "/auth/login").permitAll().anyRequest().authenticated()).exceptionHandling(i->i.authenticationEntryPoint(connectJWTAuthEntryPoint))
